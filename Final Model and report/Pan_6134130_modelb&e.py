...
# Function to get data for the basic model
def get_data_b():
...
# Function to get data for the model with copper
def get_data_e():
    n_months, n_products, n_suppliers = 12, 3, 5
    demand = np.array([
        [25, 25, 0, 0, 0, 50, 12, 0, 10, 10, 45, 99],
...
# Function to define the model with copper
...
# Function to set data and solve the basic model
def set_data_and_solve(model, x, I, z, demand, holding_costs, supplier_costs, capacity, supply_limit, Cr, Ni, Cr_required, Ni_required):
    ...
    if model.status == GRB.OPTIMAL:
        ...
        # Create dataframes for the solution
...
# Function to set data and solve the model with copper
def set_data_and_solve_with_copper(model, x, I, z, y, Cu_removed, demand, holding_costs, supplier_costs, capacity, supply_limit, Cr, Ni, Cr_required, Ni_required, CopperLimit, Cu):
    ...
    for t in range(n_months):
        for p in range(n_products):
            model.addConstr(gp.quicksum(Cu[s] / 100 * z[p, s, t] for s in range(n_suppliers)) - Cu_removed[p, t] <= CopperLimit[t] / 100 * (x[p, t] - Cu_removed[p, t]))
    ...
    if model.status == GRB.OPTIMAL:
        ...
        electrolysis_fixed_costs_total = 0
        electrolysis_variable_costs_total = 0
        # Extract the solution
        for p in range(n_products):
            for t in range(n_months):
                production_plan[p, t] = x[p, t].x
        ...
        for t in range(n_months):
            ...
            electrolysis_fixed_costs_total += 100 * y[t].x
            for p in range(n_products):
                copper_removed_plan[p, t] = Cu_removed[p, t].x
                electrolysis_variable_costs_total += 5 * Cu_removed[p, t].x
        electrolysis_costs_total = electrolysis_fixed_costs_total + electrolysis_variable_costs_total
        # Create dataframes for the solution
        products = [f'Product {p + 1}' for p in range(n_products)]
        suppliers = [f'Supplier {s + 1}' for s in range(n_suppliers)]
        months = [f'Month {t + 1}' for t in range(n_months)]
        ...
        print(f"Total Electrolysis Costs: {electrolysis_costs_total} EURO (Fixed: {electrolysis_fixed_costs_total} EURO, Variable: {electrolysis_variable_costs_total} EURO)")
    else:
        print("No optimal solution found.")


# Main function to choose and run the appropriate model
